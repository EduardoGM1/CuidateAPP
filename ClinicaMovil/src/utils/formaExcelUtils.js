 ...
 * Compartido con la app web; en móvil se guarda el archivo con `saveFormaExcelToDevice`.
 */
...
/**
 * Guarda el Excel en almacenamiento del dispositivo (Descargas / Documents).
 * @param {object} data - { cabecera, filas } desde API forma-lista
 * @param {string} filename - Nombre del archivo (.xlsx)
 * @returns {Promise<string>} Ruta absoluta del archivo guardado
 */
export async function saveFormaExcelToDevice(data, filename) {
  ...
  const safeName = String(filename || 'export.xlsx').replace(/[^\w.\-]/g, '_');
