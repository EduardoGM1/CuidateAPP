...
import { usePacienteMedicalData, usePacienteRedApoyo, usePacienteEsquemaVacunacion, usePacienteComorbilidades, usePacienteDeteccionesComplicaciones } from '../../hooks/usePacienteMedicalData';
...
const DetallePacienteContent = ({ route, navigation }) => {
  ...
  const openDeteccionModal = useCallback((det = null) => {
    const target = det || (detecciones && detecciones.length > 0 ? detecciones[0] : null);
    if (!target) {
      Alert.alert('Sin complicaciones', 'No hay complicaciones para editar.');
      return;
    }
    setEditingDeteccion(target);
    setFormDeteccion({
      tipo_complicacion: target.tipo_complicacion || target.Comorbilidad?.nombre_comorbilidad || '',
      fecha_deteccion: target.fecha_deteccion || new Date().toISOString().split('T')[0],
      observaciones: target.observaciones || '',
      exploracion_pies: !!target.exploracion_pies,
      exploracion_fondo_ojo: !!target.exploracion_fondo_ojo,
      realiza_auto_monitoreo: !!target.realiza_auto_monitoreo,
      auto_monitoreo_glucosa: !!target.auto_monitoreo_glucosa,
      auto_monitoreo_presion: !!target.auto_monitoreo_presion
    });
  ...
  const handleSaveDeteccion = useCallback(async () => {
    ...
    if (!editingDeteccion) {
      Alert.alert('Sin selección', 'No hay complicación seleccionada para editar.');
      return;
    }
    try {
      const gestionService = (await import('../../api/gestionService')).default;
      await gestionService.updatePacienteDeteccionComplicacion(pacienteId, editingDeteccion.id_deteccion, formDeteccion);
      await refreshDetecciones();
      setShowDeteccionModal(false);
      Logger.success('Detección guardada');
    } catch (err) {
    ...
    <SafeAreaView style={styles.container}>
                    ...
                    {citas.map((cita, index) => (
                      ...
                      >
                        ...
                        <Text style={styles.citaOptionDate}>
                          👨‍⚕️ Dr. {cita.doctor_nombre || 'Sin doctor'}
                        </Text>
                      </TouchableOpacity>
                    ))}
                  </ScrollView>
                ...
                <View style={styles.vitalSection}>
                  ...
                  <View style={styles.vitalsGrid}>
                    ...
                    {signo.colesterol_mg_dl && (
                      <View style={styles.vitalItem}>
                        <Text style={styles.vitalLabel}>Colesterol:</Text>
            ...
            {loadingCitaDetalle ? (
              ...
              >
                ...
                <View style={styles.modalListItem}>
                  ...
                  {citaDetalle.Doctor && (
                    <Text style={styles.modalListItemSubtitle}>
                      👨‍⚕️ Dr. {citaDetalle.Doctor.nombre} {citaDetalle.Doctor.apellido_paterno || ''}
                    </Text>
                  )}
                  ...
                  <View style={{ marginTop: 16, paddingTop: 16, borderTopWidth: 1, borderTopColor: '#e0e0e0', gap: 10 }}>
                    ...
                    {/* Botón secundario: Completar Consulta Completa (Formulario tradicional) */}
                    <TouchableOpacity
                      style={{
                      ...
                      onPress={() => handleOpenConsultaCompletaExistente(citaDetalle.id_cita)}
                    >
                      <Text style={{ color: '#fff', fontWeight: '700', fontSize: 16 }}>
                    ...
                    <Text style={{ fontSize: 11, color: '#666', textAlign: 'center', marginBottom: 8 }}>
                      Registrar signos vitales, diagnóstico y plan de medicación en un solo paso
                    </Text>
        ...
        >
          ...
          >
            ...
            >
              {/* Selección de Doctor */}
              <View style={styles.formSection}>
                <Text style={styles.label}>👨‍⚕️ Doctor (Opcional)</Text>
                <ScrollView 
                  horizontal 
            ...
            >
              ...
              <View style={styles.formSection}>
                ...
                {modoConsulta === 'completar_existente' ? (
                  ...
                  <>
                    <Text style={styles.label}>👨‍⚕️ Doctor (Requerido)</Text>
                    <ScrollView 
                      horizontal 
                      showsHorizontalScrollIndicator={false}
                      style={styles.doctoresList}
      ...
      <OptionsModal
        ...
        options={[
          {
            icon: 'plus',
            label: 'Modificar Complicaciones',
            onPress: () => {
      ...
      >
        <View style={{ gap: 12 }}>
          ...
          />
          <TextInput
            label="Fecha de detección (YYYY-MM-DD)"
            value={formDeteccion.fecha_deteccion}
            onChangeText={(text) => setFormDeteccion(prev => ({ ...prev, fecha_deteccion: text }))}
            mode="outlined"
          />
          <TextInput
            label="Observaciones"
          ...
          <View style={{ flexDirection: 'row', justifyContent: 'flex-end', gap: 12, marginTop: 8 }}>
            <Button mode="text" onPress={() => setShowDeteccionModal(false)}>Cancelar</Button>
            <Button mode="contained" onPress={handleSaveDeteccion} buttonColor="#2196F3">
      ...
      <FormModal
        ...
        title={editingComorbilidad ? '✏️ Editar Comorbilidad' : '🏥 Agregar Comorbilidad'}
      ...
      >
        ...
        >
          ...
          >
            <View style={styles.modalHeader}>
              <Title style={styles.modalTitle}>👨‍⚕️ Asignar Doctor</Title>
        ...
        >
          ...
          >
            ...
            >
              ...
              <View style={styles.formSection}>
                <Text style={styles.label}>👨‍⚕️ Doctor Nuevo *</Text>
                <ScrollView 
                  horizontal 
                  showsHorizontalScrollIndicator={false}
                  style={styles.doctoresList}
